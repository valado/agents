...
export class AgentParser {
  ...
  private hasAgentFiles(dir: string): boolean {
    try {
      ...
      if (hasMarkdownFiles) return true;
  ...
  private extractCategoryFromPath(relativePath: string): string {
  ...
  async searchAgents(options: AgentSearchOptions): Promise<Agent[]> {
    ...
    if (options.keywords) {
      ...
      results = results.filter(
        (agent) =>
          agent.metadata.name.toLowerCase().includes(keywords) ||
  ...
  async getStats() {
    ...
    const stats = {
      ...
      byCategory: {} as Record<AgentCategory, number>,
    };

    for (const category of categories) {
      stats.byCategory[category] = this.agents.filter(
